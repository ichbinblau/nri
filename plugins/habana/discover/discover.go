...
func FakeDeviceInfo(d string, u string) (*DevInfo, error) {
	major := uint32(0)
	minor := uint32(0)
	if d == "/dev/accel/accel_controlD" {
		major = 505
		idx, err := strconv.Atoi(u)
		if err != nil {
			panic(err)
		}
		minor = uint32(idx)*2 + 1
	} else if d == "/dev/accel/accel" {
		major = 505
		idx, err := strconv.Atoi(u)
		if err != nil {
			panic(err)
		...
		minor = uint32(idx) * 2
	} else if strings.HasPrefix(d, "/dev/infiniband/uverbs") {
		major = 231
		minor = 200
	}

	return &DevInfo{
		Path:     fmt.Sprintf("%s%s", d, u),
		Major:    major,
		Minor:    minor,
		Mode:     8630,
		Uid:      0,
		Gid:      0,
		FileMode: os.FileMode(8630),
	}, nil
}
